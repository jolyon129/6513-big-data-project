...
spark = SparkSession.builder.appName("task1-sql").config("spark.some.config.option",
...
open.createOrReplaceTempView("ov")

result = spark.sql("""
SELECT P.summons_number, P.plate_id, P.violation_precinct, P.violation_code, P.issue_date 
FROM park P LEFT JOIN ov O 
ON P.summons_number = O.summons_number 
WHERE O.summons_number is NULL
""")
