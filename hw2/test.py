...
if __name__ == "__main__":
    ...
    query = "SELECT * FROM (select iter.attr as attr1, iter.val as val1, vdectable.attr as attr2, vdectable.val as val2, count(*) as supportspecific " \
            "FROM (select uid, attr, val from Pro_Publica where attr<>'vdecile') iter LEFT JOIN (SELECT * FROM Pro_Publica WHERE attr='vdecile') vdectable " \
            "where iter.uid=vdectable.uid GROUP BY iter.attr, iter.val, vdectable.attr, vdectable.val) inti WHERE inti.supportspecific>500"
    F2 = spark.sql(query);
    F2.createOrReplaceTempView("F2")

    query = "select V.attr1 , V.val1 ,V.attr2, V.val2, V.SUPPORTSPECIFIC as supp,CAST(((V.SUPPORTSPECIFIC * 1.0)/E.supp) AS FLOAT) as  conf from F1 E inner join F2 V ON (E.attr=V.attr1 and E.val=V.val1) WHERE (V.SUPPORTSPECIFIC >" + str(
        supp) + ") and ((V.SUPPORTSPECIFIC * 1.0)/E.supp) > " + str(
        conf) + " ORDER BY V.attr1, V.val1, V.attr2 ,V.val2";
    R2 = spark.sql(query)
    ...
    query = "select E.attr1 , E.val1 ,E.attr2, E.val2, V.attr as attr3,V.val as val3, V.SUPPORTSPECIFIC as supp,CAST(((V.SUPPORTSPECIFIC * 1.0)/E.SUPPORTPARTIAL) AS FLOAT) as  conf from UI E inner join UIS V ON (E.attr1=V.attr1 and E.val1=V.val1 and E.attr2=V.attr2 and E.val2=V.val2) WHERE (V.SUPPORTSPECIFIC >" + str(
        supp) + ") and ((V.SUPPORTSPECIFIC * 1.0)/E.SUPPORTPARTIAL) > " + str(
        conf) + " ORDER BY E.attr1, E.val1, E.attr2 ,E.val2, V.attr, V.val";
    R3 = spark.sql(query)
